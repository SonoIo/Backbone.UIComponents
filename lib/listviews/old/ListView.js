...
var ListView = module.exports = AppView.extend({

	className: 'list',

	stopEvents: false,

	template: _.template(fs.readFileSync('templates/components/listviews/list_view.html', 'utf8')),
	...
	initialize: function initialize(options) {
		...
		this.events['touchmove']  = 'onTouchMove';
		this.events['touchstart'] = 'onTouchStart';
		this.events['touchend']   = 'onTouchEnd';
		...
		this.options.emptyText = options.emptyText || __('Nessun elemento');
		// Flag che indica se la lista deve scrollare
		this.options.scroll    = typeof options.scroll !== 'undefined'  ? options.scroll : true;
		this.options.hscroll   = typeof options.hscroll !== 'undefined' ? options.hscroll : false;
	...
	renderItems: function renderItems(collection) {
		...
		// Reimposto lo scroll
		var oldScrollTop = self.$el.scrollTop();
		...
		if (collection.length > 0) {
			...
			this.addItems(collection.models, function() {
				self.$el.scrollTop(oldScrollTop);
